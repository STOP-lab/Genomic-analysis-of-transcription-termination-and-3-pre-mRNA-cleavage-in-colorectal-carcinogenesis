...
sampleNames<-c("MiaPaCa2-T4ph-SNR", "Panc1-T4ph-SNR")

sampleCondition<-c("LowSensitivity", "HighSensitivity")
replicate<-c(1,1)
order<-c(1,2)
...
setwd("Replicates/SNR/Stranded_bedGraphs/")

gdzie<-getwd()
sampleFiles2 <- grep(".bedgraph",list.files(gdzie),value=TRUE)
sampleFiles2

sampleNames2<-c("MiaPaCa2-T4ph-SNR-fwd", "MiaPaCa2-T4ph-SNR-rev", "Panc1-T4ph-SNR-fwd", "Panc1-T4ph-SNR-rev")
...
bedtoolsBdgNorm<-function(x){
  Path <- "Replicates/SNR/Normalised_bedGraphs/"
...
for (i in 1:4) bedtoolsBdgNorm(i)
